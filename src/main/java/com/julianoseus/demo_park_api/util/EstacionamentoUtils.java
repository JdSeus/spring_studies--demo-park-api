package com.julianoseus.demo_park_api.util;
...
public class EstacionamentoUtils {
    ...
    public static BigDecimal calcularCusto(LocalDateTime entrada, LocalDateTime saida) {
        ...
        if (minutes <= 15) {
            ...
            long auxMinutes = minutes - 60;
            long quarters = (long) Math.ceil(auxMinutes / 15.0);
            total = PRIMEIROS_60_MINUTES + (ADICIONAL_15_MINUTES * quarters);
        }
    ...
    public static BigDecimal calcularDesconto(BigDecimal custo, long numeroDeVezes) {
        ...
        long vez_com_desconto = numeroDeVezes % 10;

        BigDecimal desconto = null;

        if((numeroDeVezes > 0) && (vez_com_desconto == 0)) {
            desconto = custo.multiply(new BigDecimal(String.format(""+DESCONTO_PERCENTUAL)));
        } else {
            desconto = new BigDecimal("0.0");
        }
