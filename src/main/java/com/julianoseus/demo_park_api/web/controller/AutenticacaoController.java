...
import com.julianoseus.demo_park_api.jwt.JwtToken;
...
public class AutenticacaoController {
    ...
    @Operation(summary = "Autenticar na API", description = "Recurso de autenticação na API",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Autenticação realizada com successo e retorno de um bearer token", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UsuarioLoginDto.class))),
                    ...
                    @ApiResponse(responseCode = "422", description = "Campos(s) Inválidos", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorMessage.class
                    ))),
            }
    )
    @PostMapping("/auth")
